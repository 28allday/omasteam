...
- **Center:** live clock
- **Right:** **Return to Gaming Mode**, bluetooth (tap for the bluetooth
  panel — power toggle, device list with tap-to-connect/pair), network (tap
  for the network panel — Wi-Fi toggle, network list with tap-to-connect +
  inline password entry), volume (tap for the volume panel — output/mic
  sliders + device picker; scroll to change, long-press to mute), battery
  (auto-hides if none), power (tap for the power panel — lock / suspend /
  log out / return to gaming / restart / shut down)
...
artifacts/bar/    omasteam-bar.qml (the QML bar) + omasteam-menu.qml (system menu)
                  ...
                  (power panel; autostart / shortcut entries are generated by
