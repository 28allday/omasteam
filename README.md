...
- **Center:** live clock
- **Right:** **Return to Gaming Mode**, network, volume (tap for the volume
  panel — output/mic sliders + device picker; scroll to change, long-press to
...
artifacts/bar/    omasteam-bar.qml (the QML bar) + omasteam-menu.qml (system menu)
                  ...
                  panel; autostart / shortcut entries are generated by install.sh
                  with absolute paths)
```
