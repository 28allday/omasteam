...
- **Center:** live clock
- **Right:** **Return to Gaming Mode**, network (tap for the network panel —
  Wi-Fi toggle, network list with tap-to-connect + inline password entry),
  volume (tap for the volume panel — output/mic sliders + device picker;
  scroll to change, long-press to mute), battery (auto-hides if none), power
...
artifacts/bar/    omasteam-bar.qml (the QML bar) + omasteam-menu.qml (system menu)
                  ...
                  panel) + omasteam-network.qml (network panel; autostart /
                  shortcut entries are generated by install.sh with absolute paths)
```
