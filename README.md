...
```
 Apps · Capture · Style · Setup · System
```
...
- **Apps** KRunner · **Capture** screenshot region/window/full + screen record
  (Spectacle) · **Style** next wallpaper + theme switcher (lists installed
  themes) · **Setup** Wi-Fi / Bluetooth / Audio / Display / Night Light (KDE
  panels) · **System** Lock / Suspend / Log Out / Restart / Shut Down / Return
  to Gaming.
...
`Meta+Alt+Space` system menu (with `--with-bar`).
...
bin/              omasteam-theme, theme (wrapper), omasteam-rebind-shortcuts,
                  ...
                  omasteam-menu (system-menu launcher + action dispatcher)
artifacts/        static config files copied into place (kitty.conf, service menu, …)
artifacts/bar/    omasteam-bar.qml (the QML bar) + omasteam-menu.qml (the QML
                  system menu; autostart / shortcut entries are generated by
                  install.sh with absolute paths)
```
